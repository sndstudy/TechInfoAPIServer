...
class QiitaDbAccess extends base_db_access_1.BaseDbAccess {
    async selectItems(targetSeconds, tagName) {
        ...
        const docRef = db.collection("qiita").doc(tagName);
        ...
        const returnItems = [];
        if (selectResult) {
            const data = await db.collection("qiita").doc(tagName).collection(selectResult.id).get().catch(() => { throw new Error("Firebase エラー"); });
            ;
